...
static struct {
    ...
    // XX_GPIO( 0) FastOut_1 bank 2, pin 14 (pin 21, SSP1_DATA0)
    ...
    // XX_GPIO( 1) FastOut_2 bank 0, pin 17 (pin 131, GPMI_CE1N)
    ...
    // XX_GPIO( 2) FastOut_3 bank 2, pin 12 (pin 11, SSP1_SCK)
    ...
    // XX_GPIO( 3) FastOut_4 bank 3, pin 6 (pin 78, AUART1_CTS)
    ...
    // XX_GPIO( 4) FastIn_1 bank 2, pin 20 (pin 7, SSP2_SS1)
    ...
    // XX_GPIO( 5) FastIn_2 bank 3, pin 2 (pin 70, AUART0_CTS)
    ...
    // XX_GPIO( 6) FastIn_3 bank 3, pin 4 (pin 81, AUART1_RX)
    ...
    // XX_GPIO( 7) FastIn_4 bank 3, pin 5 (pin 65, AUART1_TX)
...
void xx_gpio_config(unsigned n, int output)
{
    if (xx_base_ptr && n < XX_GPIO_MAX) {
        ...
        if (output)
            reg_ptr = (unsigned *)(xx_base_ptr + xx_gpio_doe[n].offset + XX_GPIO_SET_OFFSET);
        else
            reg_ptr = (unsigned *)(xx_base_ptr + xx_gpio_doe[n].offset + XX_GPIO_CLR_OFFSET);
...
void xx_gpio_set(unsigned n)
{
    if (xx_base_ptr && n < XX_GPIO_MAX) {
        ...
        reg_ptr = (unsigned *)(xx_base_ptr + xx_gpio_dout[n].offset + XX_GPIO_SET_OFFSET);
...
void xx_gpio_clr(unsigned n)
{
    if (xx_base_ptr && n < XX_GPIO_MAX) {
        ...
        reg_ptr = (unsigned *)(xx_base_ptr + xx_gpio_dout[n].offset + XX_GPIO_CLR_OFFSET);
        *reg_ptr = xx_gpio_dout[n].mask;
	}
}
...
{
    if ((xx_base_ptr == NULL) || (n >= XX_GPIO_MAX))
		return 0;

    return (*((unsigned *)(xx_base_ptr + xx_gpio_din[n].offset)) & xx_gpio_din[n].mask) ? 1 : 0;
}

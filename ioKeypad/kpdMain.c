...
IEC_UINT kpdMain(void *pPara)
{
	...
	uResInit = kpdInitialize(uIOLayer);
	TR_RET(uResInit);
	...
	for ( ; ; )
	{
		...
		switch(Message.uID)
		{
			case MSG_IO_CONFIG: /* -------------------------------------------- */
			{
				 ...
				 */
				uRes = kpdNotifyConfig(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_START:	/* -------------------------------------------- */
			{
				...
				uRes = kpdNotifyStart(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_STOP:	/* -------------------------------------------- */
			{
				...
				uRes = kpdNotifyStop(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_SET: /* ---------------------------------------- */
			{
				...
				uRes = kpdNotifySet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_GET: /* ---------------------------------------- */
			{
				...
				uRes = kpdNotifyGet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);

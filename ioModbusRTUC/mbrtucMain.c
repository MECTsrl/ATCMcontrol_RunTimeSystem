...
/*
 * Filename: mbrtucMain.c
 */


/* ----  Local Defines:   ----------------------------------------------------- */

#define __4CFILE__	"mbrtucMain.c"
...
IEC_UINT mbrtucMain(void *pPara)
{
	...
	uResInit = mbrtucInitialize(uIOLayer);
	TR_RET(uResInit);
	...
	osTrace("--- IO%d: waiting message\n", uIOLayer);
		...
		switch(Message.uID)
		{
			case MSG_IO_CONFIG: /* -------------------------------------------- */
			{
				 ...
				 */
				uRes = mbrtucNotifyConfig(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_START:	/* -------------------------------------------- */
			{
				...
				uRes = mbrtucNotifyStart(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_STOP:	/* -------------------------------------------- */
			{
				...
				uRes = mbrtucNotifyStop(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_SET: /* ---------------------------------------- */
			{
				...
				uRes = mbrtucNotifySet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_GET: /* ---------------------------------------- */
			{
				...
				uRes = mbrtucNotifyGet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_TERMINATE:	/* ---------------------------------------- */
			{
				...
				uRes = mbrtucFinalize(uIOLayer, &IO);
				TR_RET(uRes);

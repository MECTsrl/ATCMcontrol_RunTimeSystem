...
IEC_UINT udpMain(void *pPara)
{
	...
	uResInit = udpInitialize(uIOLayer);
	TR_RET(uResInit);
	...
	osTrace("--- IO%d: waiting message\n", uIOLayer);
		...
		switch(Message.uID)
		{
			case MSG_IO_CONFIG: /* -------------------------------------------- */
			{
				 ...
				 */
				uRes = udpNotifyConfig(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_START:	/* -------------------------------------------- */
			{
				...
				uRes = udpNotifyStart(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_STOP:	/* -------------------------------------------- */
			{
				...
				uRes = udpNotifyStop(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_SET: /* ---------------------------------------- */
			{
				...
				uRes = udpNotifySet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_GET: /* ---------------------------------------- */
			{
				...
				uRes = udpNotifyGet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);

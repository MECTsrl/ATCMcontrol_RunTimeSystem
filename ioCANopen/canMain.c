...
IEC_UINT canMain(void *pPara)
{
	...
	uResInit = canInitialize(uIOLayer);
	TR_RET(uResInit);
	...
	for ( ; ; )
	{
		...
		switch(Message.uID)
		{
			case MSG_IO_CONFIG: /* -------------------------------------------- */
			{
				 ...
				 */
				uRes = canNotifyConfig(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_START:	/* -------------------------------------------- */
			{
				...
				uRes = canNotifyStart(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_STOP:	/* -------------------------------------------- */
			{
				...
				uRes = canNotifyStop(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_SET: /* ---------------------------------------- */
			{
				...
				uRes = canNotifySet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_GET: /* ---------------------------------------- */
			{
				...
				uRes = canNotifyGet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);

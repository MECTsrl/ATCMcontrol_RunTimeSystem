...
/*
 * Filename: mbtcpsMain.c
 */


/* ----  Local Defines:   ----------------------------------------------------- */

#define __4CFILE__	"mbtcpsMain.c"
 ...
 *
 */
IEC_UINT mbtcpsMain(void *pPara)
{
	SIOConfig		IO;
	SIOLayerIniVal	INI;
	...
	uResInit = mbtcpsInitialize(uIOLayer);
	TR_RET(uResInit);

#if defined(RTS_CFG_IO_TRACE)
	osTrace("--- IO%d: Entering in main loop message\r\n", uIOLayer);
#endif
	...
	osTrace("--- IO%d: waiting message\n", uIOLayer);
		...
		switch(Message.uID)
		{
			case MSG_IO_CONFIG: /* -------------------------------------------- */
			{
				 ...
				 */
				uRes = mbtcpsNotifyConfig(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_CONFIG_RES: /* ---------------------------------------- */
			{
				 ...
				 */
			  #if defined(RTS_CFG_IO_TRACE)
				osTrace("--- IO%d: Configuration status: %d vs %d uResConfig %d vs %d\n", uIOLayer, uRespQueue, IPC_Q_NONE, uResConfig, OK);
			  #endif
				if (uRespQueue != IPC_Q_NONE)
				{
			...
			case MSG_IO_START:	/* -------------------------------------------- */
			{
				...
				uRes = mbtcpsNotifyStart(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_STOP:	/* -------------------------------------------- */
			{
				...
				uRes = mbtcpsNotifyStop(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_SET: /* ---------------------------------------- */
			{
			  ...
			  #if defined(RTS_CFG_IO_TRACE)
				osTrace("--- IO%d: NotifySet\n", uIOLayer);
			  #endif
				SIONotify *pNotify = (SIONotify *)Message.pData;

				uRes = mbtcpsNotifySet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_GET: /* ---------------------------------------- */
			{
				 ...
				 */
			  #if defined(RTS_CFG_IO_TRACE)
				osTrace("--- IO%d: NotifyGet\n", uIOLayer);
			  #endif

				SIONotify *pNotify = (SIONotify *)Message.pData;

				uRes = mbtcpsNotifyGet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_TERMINATE:	/* ---------------------------------------- */
			{
				...
				uRes = mbtcpsFinalize(uIOLayer, &IO);
				TR_RET(uRes);

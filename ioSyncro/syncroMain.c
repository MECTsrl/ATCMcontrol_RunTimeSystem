...
/*
 * Filename: syncroMain.c
 */


/* ----  Local Defines:   ----------------------------------------------------- */

#define __4CFILE__	"syncroMain.c"
...
IEC_UINT syncroMain(void *pPara)
{
	...
	uResInit = syncroInitialize(uIOLayer);
	TR_RET(uResInit);
	...
	osTrace("--- IO%d: waiting message\n", uIOLayer);
		...
		switch(Message.uID)
		{
			case MSG_IO_CONFIG: /* -------------------------------------------- */
			{
				 ...
				 */
				uRes = syncroNotifyConfig(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_START:	/* -------------------------------------------- */
			{
				...
				uRes = syncroNotifyStart(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_STOP:	/* -------------------------------------------- */
			{
				...
				uRes = syncroNotifyStop(uIOLayer, &IO);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_SET: /* ---------------------------------------- */
			{
				...
				uRes = syncroNotifySet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_NOTIFY_GET: /* ---------------------------------------- */
			{
				...
				uRes = syncroNotifyGet(uIOLayer, &IO, pNotify);
				TR_RET(uRes);
			...
			case MSG_IO_TERMINATE:	/* ---------------------------------------- */
			{
				...
				uRes = syncroFinalize(uIOLayer, &IO);
				TR_RET(uRes);
